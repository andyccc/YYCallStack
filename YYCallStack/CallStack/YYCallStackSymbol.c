...
static YYMachHeaderArr *machHeaderArr = NULL;

void getMachHeader()
{
    machHeaderArr = (YYMachHeaderArr *)malloc(sizeof(YYMachHeaderArr));
    machHeaderArr->allocLength = _dyld_image_count();
    machHeaderArr->array = (YYMachHeader *)malloc(sizeof(YYMachHeader) * machHeaderArr->allocLength);
    for (uint32_t i = 0; i < machHeaderArr->allocLength; i++) {
        YYMachHeader *machHeader = &machHeaderArr->array[i];
...
YYMachHeader *getPCInMach(uintptr_t pc)
{
    if (!machHeaderArr) {
        getMachHeader();
    }
...
void findPCSymbolInMach(uintptr_t pc, YYMachHeader *machHeader, CallStackInfo *csInfo)
{
    if (!machHeader) {
        return;
    }
...
void callStackOfSymbol(uintptr_t *pcArr, int arrLen, CallStackInfo *csInfo)
{
    for (int i = 0; i < arrLen; i++) {
        YYMachHeader *machHeader = getPCInMach(pcArr[i]);
        if (machHeader) {
